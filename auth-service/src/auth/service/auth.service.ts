...
export class AuthService {
    ...
    public async register({email, password}: RegisterRequestDto): Promise<RegisterResponse> {
        ...
        auth = new Auth();
        auth.email = email;
        auth.password = await this.jwtService.encodePassword(password);
    ...
    public async login({email, password}: LoginRequestDto): Promise<LoginResponse> {
        ...
        const isPasswordValid: boolean = await this.jwtService.isPasswordValid(password, auth.password);
        if(!isPasswordValid) {
            return {status: HttpStatus.NOT_FOUND, error: ['Password is not valid.'], token: null};
        }
